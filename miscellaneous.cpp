...
void paint2(Mat &img, int max_d){
    int color_set[3][3]={{0,0,255},{255,0,0},{0,255,0}};
    for(int i=0; i < img.rows / max_d; i++){
        for(int j=0; j < img.cols / max_d; j++){
            if(blocks[i][j]){
            line(img,Point((j+1)*max_d,i*max_d),Point(j*max_d,i*max_d),Scalar( color_set[blocks[i][j]%3][0], color_set[blocks[i][j]%3][1], color_set[blocks[i][j]%3][2]), 1, 8, 0);
            line(img,Point((j+1)*max_d,i*max_d),Point((j+1)*max_d,(i+1)*max_d),Scalar( color_set[blocks[i][j]%3][0], color_set[blocks[i][j]%3][1], color_set[blocks[i][j]%3][2]), 1, 8, 0);
            line(img,Point(j*max_d,i*max_d),Point(j*max_d,(i+1)*max_d),Scalar( color_set[blocks[i][j]%3][0], color_set[blocks[i][j]%3][1], color_set[blocks[i][j]%3][2]), 1, 8, 0);
            line(img,Point((j+1)*max_d,(i+1)*max_d),Point(j*max_d,(i+1)*max_d),Scalar( color_set[blocks[i][j]%3][0], color_set[blocks[i][j]%3][1], color_set[blocks[i][j]%3][2]), 1, 8, 0);}
        }
    }
}